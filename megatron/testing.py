...
from megatron.initialize import destroy_model_parallel, initialize_model_parallel
from megatron.random import model_parallel_cuda_manual_seed


# copied from fairscale/internal/version.py
import logging
import re
...
# NOTE: neuralink added host, and port
# def dist_init(rank: int, world_size: int, filename: str, filename_rpc: str = "") -> bool:
def dist_init(rank: int, world_size: int, port) -> bool:
    """
    ...
    # NOTE: neuralink use tcp communication
    # url = "file://" + filename
    ...
    if torch_version() >= (1, 6, 0):
        ...
        # NOTE: neuralink removed rpc init
        # rpc.init_rpc(
        #     f"Test{rank}",
        #     rank=rank,
